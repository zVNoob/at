...
std::shared_ptr<type::Type> Get_Fraction_type() {
  ...
  }
  {
    ...
    func_obj->push_func(std::make_shared<InternalFunction>(on_equal),{Get_Fraction_type(),Get_Fraction_type()});
    type->members["=="] = func_obj;
  }
  {
    auto func_obj = std::make_shared<TypedFunction>(integer::Get_Integer_type());
    func_obj->push_func(std::make_shared<InternalFunction>(on_less),{Get_Fraction_type(),Get_Fraction_type()});
    type->members["<"] = func_obj;
  }
  {
    auto func_obj = std::make_shared<TypedFunction>(integer::Get_Integer_type());
    func_obj->push_func(std::make_shared<InternalFunction>(on_greater),{Get_Fraction_type(),Get_Fraction_type()});
    type->members[">"] = func_obj;
  }
  {
    auto func_obj = std::make_shared<TypedFunction>(Get_Fraction_type());
